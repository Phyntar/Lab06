...
{
	double iso, isoDis,isoTemp;
	for (int i = 0; i < number.size(); i++)
	{
		if (i == 0)
		{
			iso = number.at(i);
			isoDis = number.at(i + 1) - number.at(i);
		}
		else if (i == number.size() - 1)
		{
			if (isoDis < number.at(i) - number.at(i - 1))
			{
				iso = number.at(i);
				isoDis = number.at(i) - number.at(i - 1);
		...
		else
		{
			...
			{
				isoTemp = number.at(i) - number.at(i - 1);
			}
			else
			{
				isoTemp = number.at(i + 1) - number.at(i);
			}
...
unmatched(list<string> & A, list<string> & B)
{
	...
	{
		bool contain = false;
		contain = (find(B.begin(), B.end(), atemp.front()) != B.end());
		if (contain == false)
		{
			count++;
		}
